...
class ReflexAgent(Agent):
  ...
  def _eatEvaluationFunction(self, currentGameState, action):
    ...
    if successorMin == currentMin:
      if self._isCorner(successorGameState,
  ...
  def chaseEvaluationFunction(self, currentGameState, action):
    ...
    if nearGhostPos[0] - 0.5 <= floatNewPos[
        0] <= nearGhostPos[0] + 0.5 and nearGhostPos[1] - 0.5 <= floatNewPos[
            1] <= nearGhostPos[1] + 0.5:
      return float("Inf")
  ...
  def _hasWallBtw(self, gameState, pacmanPos, ghostPos):
    ...
    if ghostPos[0] < pacmanPos[0] and gameState.hasWall(pacmanPos[0] - 1,
                                                        pacmanPos[1]):  # left
